...
public class QualysGetContainerService {

	private final RestTemplate restTemplate;
	Logger LOGGER = LoggerFactory.getLogger(QualysGetContainerService.class);
	//	int COUNTER = 1;

	String response;
	@Retryable(value = { RestClientException.class,RuntimeException.class }, maxAttempts = 4, backoff = @Backoff(multiplier = 3))
	public String getContainerById(String containerId) {
		response = "No Container"; //[highly unlikely] reset response to prevent assigned value of previous retry
		LOGGER.info("Invoked getContainerById");

		URI url = URI.create("https://qualysapi.qg1.apps.qualys.in/csapi/v1.1/containers/"+containerId);
		try {
	...
	public String recover() {
		LOGGER.info(" [QualysGetContainerService] Qualys Connecton failed: Audit to be implemented" + response);
		return response;
		// [TODO] REF
		// https://stackoverflow.com/questions/50322126/spring-retry-method-annotated-with-recover-not-being-called
