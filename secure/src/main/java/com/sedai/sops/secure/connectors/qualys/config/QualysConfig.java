...
public class QualysConfig {
	...
	private HttpHeaders header() {
		...
		LOGGER.info("Create header :",httpHeaders.toString());
		return httpHeaders;
	}

	@Bean (name="qualysRestTemplate")
	public RestTemplate qualysRestTemplate() {

		/* Chuck idea to timeout restTemplate due to following 
		 * https://stackoverflow.com/questions/47467663/java-9-httpclient-java-lang-noclassdeffounderror-jdk-incubator-http-httpclient
		 * 
		...
		qualysRestTemplate.setInterceptors(List.of(new qualysRestTemplateInterceptor()));
		LOGGER.info("Created RestTemplate Bean for Qualys :");
